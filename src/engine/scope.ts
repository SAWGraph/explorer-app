...
// engine refuses (docs/plans/drafts/2026-09-13-query-execution-architecture.md).
//
...
async function probeSide(
  ...
  const rows = await executeSparql(
    endpoint,
    buildEntityProbeQuery(block, regionCodes, PROBE_LIMIT),
    { cache: true },
  );
  const iris = [...new Set(rows.map((r) => r.iri).filter(Boolean))];
  // A full probe means "too many to enumerate" — we only know there are ≥200.
  if (iris.length >= PROBE_LIMIT || iris.length < 2) return null;
  return iris;
}
